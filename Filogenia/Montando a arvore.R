...
setwd("C:/Users/ggalv/Desktop/gabi/msc/analises/filogenia")
...
#sem as politomia de familia
write.tree(inserindo31, "todassp_nomesubstituido_semtamanhotempo.tre")

#com as polit de familia
write.tree(inserindo38, "tree_completa_ajeitar_tempo.tre")

nomesspprasubstituir <- inserindo38$tip.label

write.csv(nomesspprasubstituir, "nomesprasubstituir2.csv")


#pegar do copiar
nomes <- as.data.frame(read.table("clipboard", h=F))

#substituir nos tips
tips <- inserindo38$tip.label
dat <- data.frame(tips,nomes)
newtree <- sub.taxa.label(inserindo38, dat)
newtree$tip.label
